  ...
  async function searxSearch(q, page) {
    ...
    return { results, totalhits: results.length ? 1000000 : 0 };
  }

  // ── Wikipedia extract for Knowledge Panel ──
  async function getWikiData(q) {
    try {
      ...
      const items = (searchData.query && searchData.query.search) || [];
      if (!items.length) return null;
      const title = items[0].title;

      // Fetch extract + thumbnail + page URL
  ...
  async function runSearch(q, page) {
    ...
    try {
      // FIX: run search + Wikipedia KG in parallel
      const [searchResult, wikiData] = await Promise.all([
        searxSearch(q, currentPage),
        getWikiData(q)
      ]);
      const { results, totalhits } = searchResult;

      // FIX: show KG card on both mobile and desktop (Wikipedia-powered, not Serper KG)
      ...
      // FIX: load side images on both mobile and desktop
      loadSideImages(q);
    } catch (err) {
