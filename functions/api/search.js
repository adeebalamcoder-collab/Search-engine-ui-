...
async function fetchSerper(apiKey, q, type, page) {
  // Images/news/videos tabs ke liye alag endpoint
  if (type === "images" || type === "news" || type === "videos") {
    ...
    const data = await res.json();
    data.searchParameters = { q, type, page, engine: "serper", num: 100 };
    return data;
  }

  // Web search — ek hi call mein organic + news + videos + topStories sab
  ...
  // Sab results ek organic array mein merge karo
  const seen = new Set();
  const merged = [];

  const addResults = (arr, snippetKey = "snippet") => {
    (arr || []).forEach(item => {
      ...
      merged.push({
        title:       item.title   || "",
        link:        url,
        snippet:     item[snippetKey] || item.snippet || item.description || "",
        displayLink: (() => { try { return new URL(url).hostname; } catch { return url; } })(),
  ...
  addResults(data.organic);
  addResults(data.topStories, "snippet");
  addResults(data.news, "snippet");
  addResults(data.videos, "snippet");

  if (merged.length === 0) throw new Error("Serper empty results");

  return {
    organic: merged,
    knowledgeGraph: data.knowledgeGraph || null,
    searchParameters: { q, type, page, engine: "serper", num: merged.length },
...
export async function onRequestPost(context) {
  ...
  console.log('[KV]', kv ? 'bound' : 'NOT bound');
