...
async function fetchInstance(baseUrl, q, type, page) {
  ...
  try {
    ...
    // Must have results array (even if empty means it parsed correctly)
    if (!Array.isArray(data.results)) throw new Error("No results array");

    // Must have at least 1 result to count as success
    if (data.results.length === 0) throw new Error("Zero results");
