...
const CACHE_TTL = 60 * 60 * 24 * 15; // 15 days in seconds
...
export async function onRequestPost(context) {
  ...
  // ─── KV CACHE CHECK ──────────────────────────────────────────────────────
  // CF Dashboard → Pages → Settings → Functions → KV bindings
  // Variable name: SEARCH_CACHE  |  Namespace: (jo bhi banao)
  const kvKey = `s:${type}:${page}:${q}`;
  if (env.SEARCH_CACHE) {
    const hit = await env.SEARCH_CACHE.get(kvKey);
    if (hit) {
  ...
  // ─── CACHE + RETURN HELPER ────────────────────────────────────────────────
  function serveAndCache(data, engine) {
    const body = JSON.stringify(data);
    if (env.SEARCH_CACHE) {
      context.waitUntil(
        env.SEARCH_CACHE.put(kvKey, body, { expirationTtl: CACHE_TTL })
      );
    }
  ...
  if (env.LANGSEARCH_API_KEY) {
    try {
      ...
      return serveAndCache(data, "langsearch");
  ...
  if (env.SERPER_API_KEY) {
    try {
      ...
      return serveAndCache(data, "serper");
  ...
  if (env.TAVILY_API_KEY) {
    try {
      ...
      return serveAndCache(data, "tavily");
  ...
  if (env.GOOGLE_API_KEY && env.GOOGLE_CX) {
    try {
      ...
      return serveAndCache(data, "google");
  ...
  if (env.SEARXNG_URL) {
    try {
      ...
      return serveAndCache(data, "searxng");
