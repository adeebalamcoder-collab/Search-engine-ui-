...
// Path: functions/api/image.js
// Primary:   Google CSE Images (env.GOOGLE_API_KEY + env.GOOGLE_CX) — 100/day free
// Fallback1: Unsplash (env.UNSPLASH_ACCESS_KEY)
// Fallback2: Pexels (env.PEXELS_API_KEY)
// Fallback3: Tavily (env.TAVILY_API_KEY)
// Fallback4: Serper (env.SERPER_API_KEY)
// Fallback5: LangSearch (env.LANGSEARCH_API_KEY)
...
// ─── GOOGLE CSE IMAGES ─────────────────────────────────────────────────────
async function fetchGoogleImages(apiKey, cx, q, page) {
  ...
  if (items.length === 0) throw new Error("Google CSE empty images");
  ...
  return items.map((item, idx) => ({
    ...
    source:       (() => {
      try { return new URL(item.image?.contextLink || item.link).hostname; } catch { return ""; }
    })(),
...
export async function onRequestPost(context) {
  ...
  // 1️⃣ Google CSE Images (Primary — 100/day free)
  if (env.GOOGLE_API_KEY && env.GOOGLE_CX) {
    try {
      const images = await fetchGoogleImages(env.GOOGLE_API_KEY, env.GOOGLE_CX, q, page);
